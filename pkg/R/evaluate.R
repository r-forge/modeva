evaluate <-
  function(a, b, c, d, N = NULL, measure = "CCR"){
    ...
    if (measure == "CCR") { value <- (a+d)/N
      ...
      a <- as.numeric(a)  # to avoid integer overflow when large numbers (https://github.com/benjjneb/dada2/issues/1747#issuecomment-1579180381)
      b <- as.numeric(b)
      c <- as.numeric(c)
      d <- as.numeric(d)
